...
const Tarjouslomake = () => {
    ...
    const [tarjoaja, setTarjoaja] = useState({
        nimi: 'Jouni Riimala',
        osoite: 'Vuohennokantie 7, 04330 Lahela',
        puhelin: '045-2385 888',
        sahkoposti: 'jriimala@gmail.com',
        ytunnus: '3210413-8',
    });
    ...
    const laskeYhteenveto = () => {
        const yhteiskulut = tehtavat.reduce(
            (summa, item) => summa + parseFloat(item.kuluarvio),
            0
        );
    ...
    const lisaaTehtava = () => {
        ...
        if (tehtava && kuvaus && tuntiarvio) {
            ...
            setTehtavat([
                ...tehtavat,
                { tehtava, kuvaus, tuntiarvio, kuluarvio },
            ]);
            setTehtava('');
            setKuvaus('');
            setTuntiarvio(0);
        ...
        doc.autoTable({
            ...
            head: [
                [
                    ...
                    'Kuvaus',
                    'Tuntiarvio',
                    'Tuntiarvio * tuntihinta',
                    'Alv 25.5%',
                    'Alvillinen summa',
                ],
            ],
            body: tehtavat.map((item) => [
                item.tehtava,
                item.kuvaus,
                item.tuntiarvio,
                `${item.kuluarvio.toFixed(2)} €`,
                `${(item.kuluarvio * 0.255).toFixed(2)} €`,
                `${(
                    Number(item.kuluarvio) +
                    Number(item.kuluarvio) * 0.255
                ).toFixed(2)} €`,
            ]),
        });
        ...
        doc.autoTable({
            ...
            head: [
                [
                    'Kokonaissumma',
                    'ALV (25.5%)',
                    'Kotitalousvähennys (40%)',
                    'Vähennyksen jälkeen',
                ],
            ],
            body: [
                [
                    `${Number(yhteenveto.kokonaissumma)} €`,
                    `${Number(yhteenveto.alv)} €`,
                    `${yhteenveto.kotitalousvahennys} €`,
                    `${yhteenveto.maksettava} €`,
                ],
            ],
        });

        doc.text(
            'Tarjouksen kokonaisummasta maksettava ennen työnaloitusta 15%\njoka tulkitaan tarjouksen hyväksynnäksi.',
            14,
            220
        );
        doc.text(
            'Mikäli asiakas peruuttaa tilauksen maksettua 15% ei palauteta.',
            14,
            230
        );
        doc.text(
            'Ennakkomaksu tiedot: Mobile Pay (045 2385 888) / tilille FI71 1470 3500 2922 74,',
            14,
            240
        );
        doc.text(
            'lisätkää viestiosuuteen yrityksenne tai Teidän nimi.',
            14,
            245
        );
        doc.text(
            'Asiakkaan tulee myös ilmoittaa tarjouksen hyväksynnästä osoitteeseen jriimala@gmail.com',
            14,
            260
        );
    ...
    return isAccess ? (
        <div className='tarjouslomake'>
            <Container maxWidth='md'>
                <Typography variant='h4' align='center' gutterBottom>
                    Tarjouslomake - sivu {currentStep}
                </Typography>
                {currentStep === 1 && (
                    <>
                        <div className='tarjouslomake-osapuolet'>
                            <TarjoajaForm
                                tarjoaja={tarjoaja}
                                setTarjoaja={setTarjoaja}
                            />
                            <AsiakasForm saaja={saaja} setSaaja={setSaaja} />
                        </div>
                        <Button
                            className='tarjouslomake-nappi'
                            variant='contained'
                ...
                {currentStep === 4 && (
                    <div>
                        <TarjousLomake4
                            sisaltyy={sisaltyy}
                            setSisaltyy={setSisaltyy}
                            eiKuulu={eiKuulu}
                            setEiKuulu={setEiKuulu}
                            suositukset={suositukset}
                            setSuositukset={setSuositukset}
                        />
                        <div className='tarjouslomake-nappi'>
                            <Button
                                variant='contained'
        ...
        <h1 style={{ marginTop: '7rem', backgroundColor: '#FFFFFF' }}>
            Voihan räkä, asiaa tutkitaan..
        </h1>
    );
};
